...
`fetch_openml(name='iris', version=1, as_frame=True,parser='pandas')`
...
`fetch_openml(name='breast-w', version=1, as_frame=True)`
...
`fetch_openml(name='boston', version=1, as_frame=True,parser='pandas')`
...
`fetch_openml(name='california_housing', version=1, as_frame=True,parser='pandas')`

---

### 7. Auto MPG Dataset
**Description**: Predict city fuel efficiency (MPG) from car specifications such as cylinders, displacement, horsepower, and weight. Classic regression dataset.  
`fetch_openml(name='autoMpg', version=3, as_frame=True)`
"""
...
```python
features = df.columns
selected_features = # you can specify by list of columns names or select items from the features list, e.g. [features[1], features[3]..]
```
...
selected_features = ['Alcohol', 'Malic_acid', 'Ash', 'Color_intensity', 'Hue']
